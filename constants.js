export const PRODUCT_NAME = 'Layer0'
export const PRODUCT_NAME_LOWER = PRODUCT_NAME.toLowerCase()
export const PRODUCT_NAME_UPPER = PRODUCT_NAME.toUpperCase()
export const CLI_NAME = 'layer0'
export const PACKAGE_NAME = '@layer0'
export const CONFIG_FILE = 'layer0.config.js'
export const HEADER_PREFIX = 'x-layer0-'

export const DOMAIN = 'layer0.co'
export const APP_DOMAIN = `app.${DOMAIN}`
export const DOCS_DOMAIN = `developer.${DOMAIN}`
export const DOCS_PAGES_DOMAIN = 'layer0-docs.github.io'

export const APP_URL = `https://${APP_DOMAIN}`
export const FORUM_URL = `https://forum.${DOMAIN}`
export const STATUS_URL = `https://status.${DOMAIN}`
export const HELP_URL = `https://help.${DOMAIN}`
export const DOCS_PAGES_REPO_URL = `https://${DOCS_PAGES_DOMAIN}/layer0-docs-pages`

export const NODE_VERSION = '12.x'
