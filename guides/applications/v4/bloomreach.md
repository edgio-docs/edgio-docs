...
- [Caching](/applications/prefetching) configuration for GraphQL operations, API operations, and static operations have been added in the `routes.ts` file.
- To view [Prefetching](/applications/prefetching) in operation, view the `<Link />` component. This wraps menu items and category items to prefetch the content of those links to allow content to be served from instantly from the browser's cache.
...
Because this example is based off [create-react-app](https://create-react-app.dev/), you can also reference the static [react app docs](https://docs.layer0.co/applications/react) for this.
