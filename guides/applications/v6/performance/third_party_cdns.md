...
{{ PRODUCT_NAME }} offers fully featured [A/B testing](/applications/performance/traffic_splitting/a_b_testing). When {{ PRODUCT_NAME }} is running behind another CDN, the CDN must be configured in a very specific way in order for A/B testing to work:
