...
2.  [Register the desired hostname(s)](/guides/basics/hostnames#setup) within the entry environment.
...
3.  Replicate the origin configuration(s) defined within your production environment.
    ...
    2.  Paste your origin configuration(s) within the `current-site` environment.
        ...
        3.  Click **Origins Editor**. 
        4.  Verify your origin configurations. 
...
1.  Run either of the following commands from your project's root directory:

    -   **{{ PRODUCT }} {{ PRODUCT_PLATFORM }}, Cloud Functions, or Edge Functions:** Set up this environment within a different property's production environment.

        ```bash
        edgio deploy --property migration-second-variant --environment=new-site
        ```

    -   **{{ PRODUCT }} CLI:** If you do not plan on using {{ PRODUCT }} {{ PRODUCT_PLATFORM }}, Cloud Functions, or Edge Functions, then you can create an environment within the same property.

        ```bash
        {{ CLI_CMD(deploy) }} --environment new-site
        ```
