...
-   An origin server can also provide early hints through `Link` headers. Serve and cache an origin server's early hints through the [Enable Early Hints feature](#enable-early-hints).
...
Enables support for early hints derived from an origin server's response. Specifically, it allows {{ PRODUCT }} to cache `Link` response headers as early hints and serve HTTP `103 Early Hints` responses. Early hints allow clients to preload resources required by the requested resource.
...
-   This feature allows us to cache `Link` headers whose `rel` attribute is set to either:
    -   **preload:** Instructs the client to preemptively fetch and cache the asset.
    -   **preconnect:** Instructs the client to preemptively establish a connection to the origin.
-   This feature will only cache and serve a `103 Early Hints` response when the following conditions are met:

    -   The [Bypass Cache feature](#bypass-cache) has not been applied to the request.
    -   The requested content is not eligible to be cached on our network. Verify that a caching policy (e.g., [Set Max Age](#set-max-age)) has not been applied to the request.

        If your origin server sets a caching policy through the `Cache-Control` header, then you should either:

        -   Remove that header through the [Remove Origin Response Headers feature](#remove-origin-response-headers). 
        -   Set the `Cache-Control` header to `no-cache`, `no-store`, or both cache directives through the [Set Origin Response Headers feature](#set-origin-response-headers).
