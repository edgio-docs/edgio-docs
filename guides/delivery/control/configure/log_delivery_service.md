...
| egress-bytes | *(int64)* The number of total possible egress bytes for all output formats. Calculation: (raw_audio_bytes + raw_video_bytes) * raw_num_formats |
| end-time-ms | *(int64)* The request end time (in milliseconds). |
| frames | *(int32)* The number of transcoded frames. |
| ingest-bytes | *(int64)* The number of ingested bytes. If raw_is_transcode == 0 then raw_total_bytes else 0. |
...
| transcode-bytes | *(int64)* The number of transcoded bytes. If raw_is_transcode == 1 then raw_video_bytes + raw_audio_bytes else 0. |
| transcode-pixels | *(int64)*The number of transcoded pixels. If raw_is_transcode == 1 then raw_video_width * raw_video_height * raw_frames else 0. |
