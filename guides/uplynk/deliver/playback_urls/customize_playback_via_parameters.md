...
| subtitle_placeholders_off | Set to `1` to display only available language tracks, overriding the `dash_subtitles_merge` parameter. [Learn more](/uplynk/acquire#subtitle-language-tracks).<br />**Example**: `subtitle_placeholders_off=1` If your asset contains an English track and an ad contains a Spanish track, then a viewer may only select English when viewing your main content and Spanish when viewing that ad.      |
