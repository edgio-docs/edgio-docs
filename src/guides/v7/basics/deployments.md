...
- **{{ PORTAL }}:** Use this method to deploy changes made within the {{ PORTAL }}.
  ...
  1.  Load the desired environment.
      ...
      1.  From the {{ PORTAL_LINK }}, select the desired private or organization space.
...
- **{{ PRODUCT }} CLI:** Use this method to deploy changes from your local machine (e.g., changes to `{{ CONFIG_FILE }}` or {{ ROUTES_FILE }}).
  ...
  Once the deployment is complete, the CLI will output the URL for your property. The property name is automatically derived from the `name` field in `package.json`. This can be overridden by using `--property` option when running `{{ FULL_CLI_NAME }} deploy`.
...
Each time you deploy your property to {{ PRODUCT }} a deployment is created and given a unique and permanent URL based on the organization name, property name, branch name in source control, and an incrementing deployment number. If you use Git, the branch name is set by the default. If not, you can specify the `--branch` option when running `{{ FULL_CLI_NAME }} deploy`.
...
```bash
{{ FULL_CLI_NAME }} deploy my-property --token=$EDGIO_DEPLOY_TOKEN
```
...
This is an example GitHub action that will deploy your property to {{ PRODUCT }}.

For this action to work

- By default, new {{ PRODUCT }} properties are created with a `default` environment. The action below will create a new build for every push on the default environment.
- To leverage the GitHub release workflow part of the action below, you need to **create an environment** `production`.
- You need to have created a deploy key for your property (see above) and added it as a secret in your repo called "edgio_deploy_token". Read more on [accessing environment variables](/guides/basics/environments#accessing-environment-variables) which might be essential for your app during the build time and for server-side requests (including SSG/SSR).
...
#
# This GitHub action deploys your property on {{ PRODUCT }}.
#
# The property is deployed each time commits are pushed. The environment to which the changes are deployed
...
#
#
# In order for this action to deploy your property, you must:
...
Here is an example Jenkins pipeline that deploys your property to {{ PRODUCT }}:
...
- You have created a deploy key for your property and added it as an environment variable in your Jenkins configuration called "edgio_deploy_token".

```groovy
// Add this file to your project at ./Jenkinsfile
//
// This Jenkins pipeline deploys your property on {{ PRODUCT }}.
//
// The property is deployed each time commits are pushed. The environment to which the changes are deployed
// is based on the following rules:
//
...
//
// In order for this pipeline to deploy your property, you must create a deploy token from the property settings page
...
Here is an example GitLab CI/CD configuration that deploys your property to {{ PRODUCT }}:
...
- You have created environments called "staging" and "production"
- You have created a deploy key for your property and added it as a variable in your GitLab project's CI/CD settings page, named "EDGIO_DEPLOY_TOKEN"

```yml
# Add this file to your project at .gitlab-ci.yml
#
# This GitLab CI/CD configuration deploys your property on {{ PRODUCT }}.
#
# The property is deployed each time commits are pushed. The environment to which the changes are deployed
# is based on the following rules:
#
