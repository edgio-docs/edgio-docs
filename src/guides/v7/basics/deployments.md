...
- **{{ PRODUCT }} CLI:** Use this method to deploy changes from your local machine (e.g., changes to `{{ CONFIG_FILE }}` or {{ ROUTES_FILE }}).
  ...
  Once the deployment is complete, the CLI will output the URL for your property. Your property's name is automatically derived from the `name` field in `package.json`. This can be overridden by using `--property` option when running `{{ FULL_CLI_NAME }} deploy`.
