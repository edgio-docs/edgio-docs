...
When a fetch request is made, the CDN will first check the cache for a matching response. If a matching response is found, it will be served from the cache. If no matching response is found, the request will be sent to the origin and the response will be cached based on one of the following scenarios:

- The caching options specified in the `edgio` object.
- The `Cache-Control` header of the origin response.
- The default caching policy of 5 minutes if no caching options are specified.
...
#### Custom Caching Options {/* custom-caching-options */}
...
The following sample code shows how to define a custom caching policy for a fetch request. The caching options are specified in the `edgio` object, which is passed as the second argument to the `fetch()` method.
