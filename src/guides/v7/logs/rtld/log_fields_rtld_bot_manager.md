...
-   **bot_score (*Integer*):** Bot Score. (Category: Event) Indicates the request's bot score. This score indicates our level of confidence that the request originated from a bot. Returns `0` for known bots.
...
-   **captcha_status (*String*):** CAPTCHA Status. (Category: Event) Indicates the request's reCAPTCHA status. Returns `STATUS_NONE` if reCAPTCHA was not applied to the request.
