...
-   [Edge Functions](#edge-functions): Enables you to execute a small piece of JavaScript code on our edge servers.
...
This feature enables you to execute a small piece of JavaScript code on our edge servers. This code can be used to modify requests and responses as well as make additional calls to your defined origins.
...
- Each Edge Function file exports the entry point `export default async function (request, context) { < your edge function code goes here > }`.
    ...
    - `context` is a read-only object which provides additional information about the request and your environment.
        ...
        - `context.metrics` provides a set of functions for injecting metrics into your edge function. TBD: Add reference
        ...
        - `context.respondWith(response)` each edge function must call `context.respondWith(response)` to return the response from your edge function to the downstream client. TBD: Add reference
...
-   **Syntax:** `edge_function: "<file path to Javascript code, relative to project's root>"`
