...
export function getStaticPaths(guidePath: string, versioned = false) {
...
export function getStaticProps(guidePath: string, versioned = false) {
  const [root, mdPath] = guidePath.split('/');

  return async function ({params}: {params: any}) {
    ...
    if (!isVersionSpecifiedInSlug && versioned) {
      // no version specified in the path, so redirect to the latest version path
      return {
        redirect: {
          destination: `/${mdPath}/v${latestVersion}/${slug.join('/')}`,
          permanent: true,
        },
      };
    ...
    content = templateReplace(
      content,
      versioned ? getVersionedConfig(version) : getBaseConfig()
    );
