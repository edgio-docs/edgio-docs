...
- [Caching](/applications/performance/prefetching) configuration for GraphQL operations, API operations, and static operations have been added in the `routes.ts` file.
- To view [Prefetching](/applications/performance/prefetching) in operation, view the `<Link />` component. This wraps menu items and category items to prefetch the content of those links to allow content to be served from instantly from the browser's cache.
...
### Create Bloomreach Content Project {/*create-bloomreach-content-project*/}
...
Because this example is based off [create-react-app](https://create-react-app.dev/), you can also reference the static [react app docs](/applications/sites_frameworks/getting_started/react) for this.
