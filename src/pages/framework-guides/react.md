...
This guide shows you how to serve a [React](https://reactjs.org/) application on Layer0. If you're using Next.js specifically, we suggest using the [Next.js guide](/guides/next).
...
- `{{ CONFIG_FILE }}` - The main configuration file for Layer0.
...
In order to render on Layer0, you need to provide a function that takes a Node `Request` and `Response` and sends the HTML that results from the `renderToString()` method from `react-dom/server`. Configure that function using the `server` property of `{{ CONFIG_FILE }}`. Here's an example:
