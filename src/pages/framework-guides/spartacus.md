...
This guide shows you how to deploy [Spartacus](https://sap.github.io/spartacus-docs) apps on Layer0.
...
For an app called `my-layer0-spartacus-app` the Layer0 config file created by `0init` will look like so:
...
If you have several projects and the `defaultProject` as specified in `angular.json` is not the project with the SSR build, specify the correct project with the `ANGULAR_PROJECT` environment variable. For example: `ANGULAR_PROJECT=my-ssr-project 0build`.
...
The default `routes.js` file created by `0init` sends all requests to Angular server via a fallback route.
