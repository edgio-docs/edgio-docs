...
- All API requests are routed to the same _domain_. The requests are then routed to the appropriate Bloomreach API via {{ PRODUCT_NAME }}'s routing framework to either the BRXM API or the GraphQL endpoint. This is done via a combination of the `{{ CONFIG_FILE }}` file and the `routes.ts` file.
...
<Callout type="info">
  Bloomreach also provides a set of{' '}
  <a href="https://documentation.bloomreach.com/content/docs/log-in-to-the-developer-environment">
    sample credentials
  </a>{' '}
...
<Callout type="info">Change the `endpoint` url to your own instance.</Callout>
