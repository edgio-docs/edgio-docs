...
export const getStaticPaths = async () => {
  ...
  const [src, path] = srcPath.split('/');
  const allGuides = (
    await globby([`${path}/**/*.{md,mdx}`], {
  ...
  // if in dev mode, render on demand
  if (isEdgioRunDev()) {
    return {
      paths: [],
      fallback: 'blocking',
